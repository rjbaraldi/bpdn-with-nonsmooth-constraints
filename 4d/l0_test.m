...
dataclass = 'l0'; 
algtype = {'spglr', 'spglr', 'spglr', 'l0', 'l0', 'l0'};
...
switch(dataclass)
    ...
    case{'l1', 'l0'}
        sigval = 2*r*10*[1/norm(b,'fro'), 1/norm(b, 'fro'), 1/norm(b,'fro'), 1, 2, 3];
...
xinit = [vec(params.L);vec(params.R)];
sinit = [1e-3, 1e-2, 1e-1, 1, 100, 1000]; %initial was 1e-6
%numsources in pic
poolobj = parpool(splitnum);
nums = 5; 
picmat = zeros(nums*nrecx, nums*nrecy, splitnum+2); 
...
parfor i = 1:splitnum
    switch(algtype{i})
        case{'spglr'}
            ...
            [xout, r,g, infor]= spgLR(@NLfunforwardCS,vec(b),tau,sigval(i),sinit(i)*xinit,opts,params);
    ...
    picmat(:,:,i+2) = rec(1:nums*nrecx, 1:nums*nrecy) ;
end 
disp(snr)
disp(snrw)
save(['./' svfile '/' 'runinfo' dataclass],'picmat', 'snr', 'times', '-v7.3');
for i = 1:splitnum
    h1 = figure;set(h1, 'Visible', 'off');imagesc(real(squeeze(picmat(:,:,i+2))));colormap gray;caxis([-1 1]*amp);
    xlabel('source-x, receiver-x','FontSize',18, 'FontName','helvetica','FontWeight','bold');
    ylabel('source-y, receiver-y','FontSize',18, 'FontName','helvetica','FontWeight','bold');
    set(gca,'fontsize',18,'FontName','helvetica','FontWeight','bold');
    saveas(h1, ['./' svfile '/' algtype{i} '_4d_z_' dataclass '_' i],'jpg');
    h1 = figure;set(h1, 'Visible', 'off');imagesc(real(squeeze(abs(picmat(:,:,2)-picmat(:,:,i+2)))));colormap gray;caxis([-1 1]*amp);
    xlabel('source-x, receiver-x','FontSize',18, 'FontName','helvetica','FontWeight','bold');
    ylabel('source-y, receiver-y','FontSize',18, 'FontName','helvetica','FontWeight','bold');
    set(gca,'fontsize',18,'FontName','helvetica','FontWeight','bold');
    saveas(h1, ['./' svfile '/' 'diff_' algtype{i} '_4d_z_' dataclass '_' i],'jpg');
