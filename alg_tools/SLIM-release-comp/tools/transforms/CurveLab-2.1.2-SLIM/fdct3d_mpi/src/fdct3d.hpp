...
//----------------------------------------
//forward transform, x->(c,w) w/ wavelets
...
//inverse transform (c,w)->x w/ wavelets
...
//forward transform, x->c w/o wavelets
int fdct3d_forward(int N1, int N2, int N3, int nbscales, int nbdstz_coarse,
					 CpxNumTnsBlkd& x,
					 CpxCrvletPrtd& c);
//this function performs the forward curvelet transform
...
//inverse transform (c,w)->x w/o wavelets
int fdct3d_inverse(int N1, int N2, int N3, int nbscales, int nbdstz_coarse, int b,
					 CpxCrvletPrtd& c,
					 CpxNumTnsBlkd& x);
//this function performs the inverse curvelet transform
//INPUTS:
...
//  nbscales -- the total number of scales for subband decomposition
//  nbdstz_coarse -- the number of discretizations in each direction on each face in the 2nd coarest scale
//  ac -- 0 for wavelets and 1 for curvelets at finnest scales
//OUTPUTS:
...
int fdct3d_partition_cpxcrvletprtd(int N1,int N2,int N3, int nbscales,int nbdstz_coarse, int ac,
										  vector< vector<bool> >& exists, vector< vector<int> >& owners);

int fdct3d_dependency(int N1,int N2,int N3, int b, int nbscales, int nbdstz_coarse, int ac,	vector< vector<int> >& crvowners, 
						 BolNumTns& tnsexists);
