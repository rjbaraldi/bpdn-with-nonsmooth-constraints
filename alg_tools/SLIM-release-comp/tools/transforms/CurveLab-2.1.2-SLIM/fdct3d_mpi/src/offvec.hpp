...
//INPUT template <class F> inline istream& operator>>(istream& is, OffVec<F>& vec)
//OUTPUT template <class F> inline ostream& operator<<(ostream& os, const OffVec<F>& vec)
...
//template <class F> inline void clear(OffVec<F>& vec){  memset(vec.data(), 0, vec.m()*sizeof(F));}

typedef OffVec<bool>   BolOffVec;
typedef OffVec<char>   ChrOffVec;
