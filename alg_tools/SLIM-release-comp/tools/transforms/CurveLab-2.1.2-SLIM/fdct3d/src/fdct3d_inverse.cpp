...
int fdct3d_inverse_angles(double L1,double L2,double L3, int s,int nd, vector< vector<CpxNumTns> >& C, CpxOffTns& O)
{
  ...
  //map<inttriple, fftwnd_plan> planmap; //All FFTW Manipulation done in file now --cody
  ...
  for(int h=0; h<nd; h++) { //(y first z second)
	 for(int g=0; g<nd; g++) {
		  ...
		  //This handles all the FFTW manipulation --cody
		  fdct3d_fft2(csc[wcnt], wpdata, false);
  ...
  }*/  //All FFTW manipulations are done through a single file now. --cody
...
int fdct3d_inverse_clt(int N1, int N2, int N3, int nbscales, int nbdstz_coarse, int ac,
					 vector< vector<CpxNumTns> >& C, CpxOffTns& F)
{
  //fft
  CpxOffTns O;
  if(ac==1) {
  ...
  //CpxOffTns F(N1,N2,N3);  --input directly --cody
  if(ac==1) {
	 double L1 = 4.0*N1/3.0;  double L2 = 4.0*N2/3.0;  double L3 = 4.0*N3/3.0;
	 int S1, S2, S3;  int F1, F2, F3;  double R1, R2, R3;	 fdct3d_rangecompute(L1, L2, L3, S1, S2, S3, F1, F2, F3, R1, R2, R3);
	 IntOffVec t1(S1);
	 ...
	 O.resize(S1, S2, S3);
	 for(int i=-F1; i<-F1+S1; i++)		for(int j=-F2; j<-F2+S2; j++)		  for(int k=-F3; k<-F3+S3; k++)			 F(t1(i), t2(j), t3(k)) += O(i,j,k);
  } else {
	 F = O;
  }
  
  //fft
  //FFTW moved outside function to allow access directly to curvelet transformation
  
  return 0;
}


int fdct3d_inverse(int N1, int N2, int N3, int nbscales, int nbdstz_coarse, int ac,
					 vector< vector<CpxNumTns> >& C, CpxNumTns& X)
{
  CpxOffTns F(N1,N2,N3);
  
  fdct3d_inverse_clt(N1, N2, N3, nbscales, nbdstz_coarse, ac, C, F);
