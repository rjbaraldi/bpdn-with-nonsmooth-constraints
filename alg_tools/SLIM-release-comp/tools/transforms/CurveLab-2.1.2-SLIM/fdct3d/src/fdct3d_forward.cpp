...
int fdct3d_forward_angles(double L1, double L2, double L3, int s, int nd, CpxOffTns& O, vector< vector<CpxNumTns> >& C)
{
  ...
  //map<inttriple, fftwnd_plan> planmap;  //ALL FFTW info is removed to file --cody
  ...
  for(int h=0; h<nd; h++) { //(y first z second)
	 for(int g=0; g<nd; g++) {
	   ...
	   fdct3d_fft2(csc[wcnt],wpdata,true);
  ...
  }*/  //all FFTW manipulation handled in file now --cody
...
int fdct3d_forward_wavelet(double L1, double L2, double L3, int s, CpxOffTns& O, vector< vector<CpxNumTns> >& C)
{
  ...
  //All FFTW manipulation is done in file now, clean map after fft2 --cody
  fdct3d_fft2(csc[0], O, true);
  fftw_cleanmap();
...
//-----------------------------------------------
int fdct3d_forward_clt(int N1, int N2, int N3, int nbscales, int nbdstz_coarse, int ac,
				   CpxOffTns& F, vector< vector<CpxNumTns> >& C)
{
  //All FFTW manipulation is done before for direct access to Curvlet Transformation --cody
				   ...
				   CpxNumTns& X, vector< vector<CpxNumTns> >& C)
{
  ...
  fdct3d_fft(X,O,false);

  fdct3d_forward_clt(N1,N2,N3,nbscales,nbdstz_coarse,ac,O,C);
