...
int ifdct_wrapping(int N1, int N2, int nbscales, int nbangles_coarse, int allcurvelets, vector< vector<CpxNumMat> >& c, CpxNumMat& x)
{
  ...
  //Function that takes a (vector< vector<CpxNumMat>) c and will produce the Curvlet transformation.
  fdct_wrapping_invclt(N1, N2, nbscales, nbangles_coarse, allcurvelets, c, O);
  
  //Takes a CpxOffMat O and will produce the CpxNumMat x. Performs the FFTW information.
  fdct_wrapping_fft(x, O, true);

  return 0; 
}

int ifdct_wrapping_nofft(int N1, int N2, int nbscales, int nbangles_coarse, int allcurvelets, vector< vector<CpxNumMat> >& c, CpxNumMat& x)
{
  assert(nbscales==c.size() && nbangles_coarse==c[1].size());
  CpxOffMat O(N1,N2);

  // std::cout<< "nofft inverse start\n"; // debug
  
  //Function that takes a (vector< vector<CpxNumMat>) c and will produce the Curvlet transformation.
  fdct_wrapping_invclt(N1, N2, nbscales, nbangles_coarse, allcurvelets, c, O);
  
  //Takes a CpxOffMat O and will produce the CpxNumMat x. Performs the FFTW information.
  fdct_wrapping_skipfft(x, O, true);

  // std::cout<< "nofft inverse end\n"; // debug
...
int fdct_wrapping_invsepangle(double XL1, double XL2, int nbangle, vector<CpxNumMat>& csc, CpxOffMat& Xhgh)
{
  ...
  //All FFTW functions moved to single file --cody
  ...
  for(int qi=0; qi<nbquadrants; qi++) {
	 ...
	 for(int w=nd-1; w>=0; w--) {
...

		{
		  ...
		  //Load, fft and fftshift the data.  Uses planmap optimization.
		  fdct_wrapping_fft2(csc[wcnt],rpdata,false);
...
int fdct_wrapping_invwavelet(vector<CpxNumMat>& csc, CpxOffMat& Xhgh)
{
  ...
  //Perform all FFTW information on the data.
  fdct_wrapping_fft2(csc[0], Xhgh, false);
  fftw_cleanmap();
...
{
  //Function that takes a (vector< vector<CpxNumMat>) c and will produce an the curvlet Transformation.
