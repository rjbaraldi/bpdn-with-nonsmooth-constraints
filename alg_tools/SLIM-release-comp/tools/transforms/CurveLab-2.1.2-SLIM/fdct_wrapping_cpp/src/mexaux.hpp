...
template <class T1, class T2> inline void cpp2mex(const pair<T1,T2>& cd, mxArray*& md);
...
template <class T1, class T2> inline void cpp2mex(const pair<T1,T2>& cd, mxArray*& md)
{
  md = mxCreateCellMatrix(1, 2);

  mxArray* ss1;	 cpp2mex(cd.first, ss1);
  mxSetCell(md, 0, ss1);

  mxArray* ss2;	 cpp2mex(cd.second, ss2);
  mxSetCell(md, 1, ss2);
