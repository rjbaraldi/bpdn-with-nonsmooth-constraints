...
int fdct_wrapping(int N1, int N2, int nbscales, int nbangles_coarse, int allcurvelets, CpxNumMat& x, vector< vector<CpxNumMat> >& c)
{
  ...
  //Function that takes an x and will produce an FFT O.  Will perform FFTW information.
  fdct_wrapping_fft(x, O, false);
  
  //Takes a CpxOffMat O and will produce the Curvlet Vector (vector< vector<CpxNumMat>) c.
  fdct_wrapping_clt(N1, N2, nbscales, nbangles_coarse, allcurvelets, O, c);

  return 0;
}

int fdct_wrapping_nofft(int N1, int N2, int nbscales, int nbangles_coarse, int allcurvelets, CpxNumMat& x, vector< vector<CpxNumMat> >& c)
{
  assert(N1==x.m() && N2==x.n());
  CpxOffMat O(N1, N2);

  // std::cout<< "nofft forward start\n"; // debug
  
  //Function that takes an x and will produce an FFT O.  Will perform FFTW information.
  fdct_wrapping_skipfft(x, O, false);
  
  //Takes a CpxOffMat O and will produce the Curvlet Vector (vector< vector<CpxNumMat>) c.
  fdct_wrapping_clt(N1, N2, nbscales, nbangles_coarse, allcurvelets, O, c);

  // std::cout<< "nofft forward end\n"; // debug
...
int fdct_wrapping_sepangle(double XL1, double XL2, int nbangle, CpxOffMat& Xhgh, vector<CpxNumMat>& csc)
{
  ...
  //map<intpair, fftwnd_plan> planmap2 = planmap;
  //fftw stuff moved to single file --cody
  ...
  for(int qi=0; qi<nbquadrants; qi++) {
	 ...
	 for(int w=nd-1; w>=0; w--) {
		...
		//IFFT
		{
		  ...
		  //Perfrom FFTW info. Will use planMap optimization.
		  fdct_wrapping_fft2(csc[wcnt], rpdata, true);
		}
		wcnt++;
	 } //end of w loop
  } //end of q loop
  ...
  //Clean the memory of the PlanMap.
  fftw_cleanmap();
  return 0;
}
//-----------------------------------------------------------------------
int fdct_wrapping_wavelet(CpxOffMat& Xhgh, vector<CpxNumMat>& csc)
{
  //Perform the FFTW information.
  fdct_wrapping_fft2(csc[0], Xhgh, true);
  fftw_cleanmap();
